...
from ..dft_utils import rfft_shape, fftshift_2d, fftshift_3d
...
def _construct_fftfreq_grid_2d(
    ...
    last_axis_fftfreq = torch.fft.rfftfreq if rfft is True else torch.fft.fftfreq
    h, w = image_shape
    freq_y = torch.fft.fftfreq(h, d=dh, device=device)
    freq_x = last_axis_fftfreq(w, d=dw, device=device)
